...
export const translations = {
  ja: {
    ...
    privacy: {
      ...
      sections: {
        s1: "本アプリの開発者は、アプリを通じて利用者の個人情報を収集しません。解析データと設定は端末内に保存されます。iCloud同期を有効にすると、バッテリー記録は利用者のiCloudプライベートデータベースに保存され、Appleのサーバーを通じて同じApple IDの端末間で同期されます。Apple Watchアプリを使用する場合は、ペアリングしたWatchにも記録を転送します。利用者が共有やお問い合わせを実行した場合は、その操作で選んだ情報が送信されます。",
        s2: "本アプリで読み込まれたログデータや解析結果は、本アプリ内での表示および管理の目的にのみ使用されます。",
        s3: "開発者は本アプリを通じて利用者の記録を収集したり、第三者へ販売・提供したりしません。利用者の設定や操作に伴うiCloud同期、Watchへの転送、共有については第1項のとおりです。",
    ...
    terms: {
      title: "利用規約",
      intro:
        "この利用規約（以下、「本規約」といいます。）は、MochiLog（以下、「本アプリ」といいます。）の利用条件を定めるものです。ユーザーは、本アプリを利用することによって、本規約に同意したものとみなされます。",
      articles: [
        {
          id: 1,
          title: "第1条（免責事項）",
        ...
        },
        {
          ...
          title: "第5条（規約の変更）",
          paragraphs: [
            "開発者は、必要と判断した場合には、ユーザーに通知することなくいつでも本規約を変更することができるものとします。変更後の規約は、本アプリまたは配布ページ内に表示された時点から効力を生じるものとします。",
          ],
        },
        {
          id: 6,
  ...
  en: {
    ...
    privacy: {
      ...
      sections: {
        s1: "The developer does not collect personal information through the app. Analytics data and settings are stored on your device. When you enable iCloud sync, battery records are stored in your private iCloud database and synced through Apple’s servers between devices using the same Apple Account. Records are also transferred to your paired Watch when you use the Watch app. Sharing or contacting support sends the information you choose in that action.",
        ...
        s3: "The developer does not collect, sell, or provide your records to third parties through the app. iCloud sync, Watch transfers, and sharing follow your settings and actions as described above.",
    ...
    terms: {
      ...
      intro:
        'These Terms of Use (hereinafter referred to as "Terms") set forth the conditions for the use of MochiLog (hereinafter referred to as "the App"). By using the App, you are deemed to have agreed to these Terms.',
      articles: [
        {
          id: 1,
        ...
        },
        {
          ...
          paragraphs: [
            "The Developer reserves the right to modify these Terms at any time without prior notice to users. The modified Terms shall become effective once displayed within the App or on the distribution page.",
          ],
        },
        {
          id: 6,
