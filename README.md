...
`redesign/app-showcase-20260908` は、アプリの実装に合わせてデザインと内容を刷新したブランチです。**本番へのトラフィック切替とmainへのマージはまだ行っていません。** 改修版は上記のCloudflareプレビューで確認できます。
...
```sh
git clone --branch redesign/app-showcase-20260908 https://github.com/ryuya0124/MochiLog-Web.git
cd MochiLog-Web
...
This redesign branch is available at the preview link above; it has not been merged into main or switched to production traffic. Unreleased app features are marked as in development. Use `pnpm build` for a dry-run build, `pnpm test` for checks, and `pnpm run deploy` only when publishing to production.
